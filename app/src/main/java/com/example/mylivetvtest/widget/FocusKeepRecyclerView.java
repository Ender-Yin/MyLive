package com.example.mylivetvtest.widget;
...
public class FocusKeepRecyclerView extends RecyclerView {
    ...
    public void requestChildFocus(View child, View focused) {
        ...
        if (!hasFocus()) {
            //当前列表没有焦点。recyclerview 子view 重新获取焦点，调用移入焦点的事件监听
        ...
        if ( mCurrentFocusPosition == linearManager.findLastVisibleItemPosition() && canScrollVertically(1) && mLastFocusView != null){

            smoothScrollBy(0,mLastFocusView.getHeight());
            postInvalidate();
        }
        if (mCurrentFocusPosition == linearManager.findFirstVisibleItemPosition() && canScrollVertically(-1) && mLastFocusView != null){
            smoothScrollBy(0,-mLastFocusView.getHeight());
            postInvalidate();
        }*/
    }
